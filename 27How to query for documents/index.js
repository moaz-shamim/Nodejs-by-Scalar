...
async function getCourseByHitesh() {
	...
	const courses = await Course.find({ creator: "Hitesh" }).select({
		name: 1,
		publishedDate: 1,
	});
	console.log(courses);
...
async function getCourseByRating() {
	...
	const courses = await Course.find({ rating: { $eq: 4.4 } }).select({
		name: 1,
		publishedDate: 1,
	});
	console.log(courses);
...
async function getCourseWithGreaterRating() {
	...
	const courses = await Course.find({ rating: { $gt: 4 } }).select({
		name: 1,
		publishedDate: 1,
	});
	console.log(courses);
...
async function getCourseWithLessRating() {
	...
	const courses = await Course.find({ rating: { $lt: 4 } }).select({
		name: 1,
		creator: 1,
	});
	console.log(courses);
...
async function getCourseWithLessOrEqualRating() {
	...
	const courses = await Course.find({ rating: { $lte: 4.2 } }).select({
		name: 1,
		creator: 1,
	});
	console.log(courses);
