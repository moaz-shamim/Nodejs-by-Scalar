...
app.put("/users/:name", (request, response) => {
  let user = users.find((item) => item.firstName === request.params.name);

  if (!user) {
    return response
      .status(404)
      .send("The user you are searching for doesn't exist");
  }

  if (!request.body.firstName) {
    return response
      .status(400)
      .send("First name is required in the request body");
  }
...
app.delete("/users/:name", (request, response) => {
  let updatedUsers = users.filter(
    (item) => item.firstName !== request.params.name
  );
